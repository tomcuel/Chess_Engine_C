...
int main (){
    ...
    Game_Settings* game_settings = create_game_settings();
    // some time variables
    ...
    bit_board* bitboards = init_bitboards();

    // Initialize the log 
    Log* chess_log = create_log(max_size_log_array);

    // Initialize the bit_board log 
    Bitboards_Log* bitboard_log = create_bitboards_log(max_size_log_array);

    // Initialize the capture pieces and the score
    Captured_Pieces_and_Score* captured_pieces_and_score = create_captured_pieces_and_score(NUMBER_OF_PIECES_TAKENABLE_PER_COLOR);
    ...
    int size_MB = 64;
    TT* transposition_tables = initialize_transposition_tables(size_MB);
    ...
    while (is_running_game != -1){
        ...
        while(SDL_PollEvent(&event)){
            ...
            // the others keys are used to fast things for the testings
                ...
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_ESCAPE:
                        ...
                        if (is_running_game == CHESSBOARD_RENDER || is_running_game == VICTORY_MENU){
                            download_log(chess_log, bitboard_log, game_settings);
                        }
                        is_running_game=-1;
                        break;
                    ...
                    case SDLK_d:
                        ...
                        break;
                    // generate all the possible moves for the player that is playing
                    case SDLK_g:
                        if (is_running_game == CHESSBOARD_RENDER){
                            int nb_moves = 0;
                            move* possibles_moves = generate_all_possible_moves(&nb_moves, chess_log, game_settings, bitboards, NUMBER_OF_BIBOARD);
                            for (int i=0; i<nb_moves; i++){
                                printf("Move %d : %d %d\n", i, get_from_square(possibles_moves[i]), get_to_square(possibles_moves[i]));
                            }
                        }
                        break;
                    ...
                    case SDLK_o: 
                        if (is_running_game == GAMEPLAY_CHOICE){
                            ...
                            open_file_and_setup_chess_game(&has_opened, chess_log, game_settings, bitboards, NUMBER_OF_BIBOARD, bitboard_log, captured_pieces_and_score);
                            is_running_game = CHESSBOARD_RENDER;
                        }
                    ...
                    case SDLK_p:
                        ...
                        if (is_running_game == VICTORY_MENU){
                            undo_last_move(chess_log, bitboard_log, bitboards, NUMBER_OF_BIBOARD, captured_pieces_and_score, game_settings);
                        }
                        is_running_game = CHESSBOARD_RENDER;
                        break;
                    ...
                    case SDLK_r:
                        if (is_running_game == CHESSBOARD_RENDER || is_running_game == VICTORY_MENU){
                            ...
                            // reset the board
                            reset_bitboards(bitboards);
                            // reset the log
                            reset_log(chess_log);
                            reset_bitboards_log(bitboard_log);
                            // reset the captured pieces and the score
                            reset_captured_pieces_and_score(captured_pieces_and_score);
                            // reset the transposition tables
                            reset_transposition_tables(transposition_tables);
                        }
                        break;
                    ...
                    case SDLK_z:
                        if (is_running_game == CHESSBOARD_RENDER){
                            bit_board zobrist_key = get_zobrist_key(bitboards, game_settings->color_of_player_that_is_playing, chess_log->moves[chess_log->actual_size-1]->castling_rights, chess_log->moves[chess_log->actual_size-1]->move_value);
            ...
            // taking the mouse event, of realising the mouse, to avoid having a second click to do, realising the click will make the move happens (or not if not valid)
                ...
                if (is_clicked == true && is_clicked_on_board == true && game_settings->has_match_started == true && is_player_playing_human == true && game_settings->is_pawn_promotion_happening == false && is_running_game == CHESSBOARD_RENDER){
                    ...
                    else if (is_en_passant_possible(game_settings->color_of_player_that_is_playing, bitboards, NUMBER_OF_BIBOARD, from_index, to_index, chess_log)){
                    ...
                    else if (is_castle_possible(game_settings->color_of_player_that_is_playing, bitboards, NUMBER_OF_BIBOARD, from_index, to_index, chess_log)){
                        flag = Castle_Flag;
                        castling = true;
                    }
                    ...
                    if (type_move_legal_attacking != NOTHING){
                        ...
                        update_log_and_captured_piece_and_score(chess_log, captured_pieces_and_score, move_value, piece_taken_type);
                        ...
                        update_castle_state_in_log(chess_log, bitboards, NUMBER_OF_BIBOARD, from_index, to_index, flag, 1-game_settings->color_of_player_that_is_playing);
                        
                        // adding the bitboards to the log 
                        add_bitboards_to_log(bitboard_log, bitboards);
                        ...
                        int check_state_after_move = is_check(1-game_settings->color_of_player_that_is_playing, bitboards, NUMBER_OF_BIBOARD, chess_log);
        ...
        if (is_running_game == GAMEPLAY_CHOICE || is_running_game == COLOR_CHOICE || is_running_game == DIFFICULTY_CHOICE || is_running_game == DIFFICULTY_CHOICE_WHITE_AI || is_running_game == DIFFICULTY_CHOICE_BLACK_AI || is_running_game == TIME_CHOICE){
            ...
            // show the load menu
            show_load_menu(renderer, is_running_game, buttons, is_clicked, draw_red_boundary_move);
        ...
        // if we're in the game, we show the chess board
            ...
            show_menu_button_in_game(renderer, buttons, game_settings->is_pawn_promotion_happening, game_settings->color_of_pawn_promotion, game_settings->is_game_on_pause, game_settings->game_mode);
            ...
            show_captured_pieces(renderer, captured_pieces_and_score, game_settings->is_board_reversed);
            show_score(renderer, captured_pieces_and_score, game_settings->is_board_reversed);

            // show the timers
            show_updated_timer(renderer, game_settings->white_time_remaining, game_settings->black_time_remaining, game_settings->player_that_is_playing, game_settings->is_board_reversed);

            // showing the log (only if a pawn promotion is not happening)
            show_log(renderer, chess_log, bitboard_log, game_settings, bitboards, NUMBER_OF_BIBOARD, game_settings->is_pawn_promotion_happening);

            // show chess board
            show_chess_board(renderer, bitboards, NUMBER_OF_BIBOARD, chess_log, game_settings, is_clicked, draw_red_boundary_move, game_settings->is_board_reversed, selected_piece_index, piece_movement_rect);

            // show the trajectories possible for the selected piece
            show_trajectory(renderer, bitboards, NUMBER_OF_BIBOARD, game_settings, chess_log, is_clicked, is_clicked_on_board, selected_piece_index, game_settings->is_board_reversed, game_settings->color_of_player_that_is_playing);
            ...
            if (is_clicked == true && is_clicked_on_board == false){
                show_confirmation_menu(renderer, buttons);
            }
            ...
            int draw_type = is_draw_forced_by_rules(chess_log, bitboard_log, bitboards, NUMBER_OF_BIBOARD);
            int check_state = get_check_state(game_settings->color_of_player_that_is_playing, bitboards, NUMBER_OF_BIBOARD, chess_log, game_settings);
        ...
        // if we're in the victory menu, we show the victory menu
            ...
            show_victory_menu(renderer, buttons, game_settings, is_clicked, draw_red_boundary_move);
        ...
        if (is_running_game == CHESSBOARD_RENDER && game_settings->has_match_started == true && game_settings->is_game_on_pause == false && ((game_settings->color_of_player_that_is_playing == WHITE && game_settings->white_player_type == AI) || (game_settings->color_of_player_that_is_playing == BLACK && game_settings->black_player_type == AI))){
            ...
            if (get_check_state(game_settings->color_of_player_that_is_playing, bitboards, NUMBER_OF_BIBOARD, chess_log, game_settings) == CHECK_MATE){
            ...
            make_AI_play(game_settings, chess_log, bitboard_log, bitboards, NUMBER_OF_BIBOARD, captured_pieces_and_score, level_of_AI, transposition_tables);
    ...
    destroy_game_settings(game_settings);

    // free the bitboards
    destroy_bitboards(bitboards);

    // free the log
    destroy_log(chess_log);

    // free the bitboards log 
    destroy_bitboards_log(bitboard_log);

    // free the captured pieces and the score
    destroy_captured_pieces_and_score(captured_pieces_and_score);
    ...
    free_transposition_tables(transposition_tables);
