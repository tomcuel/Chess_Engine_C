...
void download_log(Log* log, Bitboards_Log* bitboard_log, Game_Settings* game_settings){
    ...
    // getting this before the loop to know the color of the player that have to play, so the next part doesn't mess with it
    int color_of_player_have_to_play = game_settings->color_of_player_that_is_playing;
    // simulating the moves (we need it to know the color of the player that have to play, to get the check state)
    game_settings->color_of_player_that_is_playing = WHITE;

    for (int i = 0; i < log->actual_size; i++){
        ...
        move move_played = log->moves[i]->move_value;
        ...
        if (get_flag(move_played) == Castle_Flag){
            ...
            if (is_promotion_flag(get_flag(move_played))){
                ...
                if (log->moves[i]->captured_piece_type != NOTHING){
            ...
            // no promotion, we do the normal thing
                ...
                int piece_moved = get_type_of_piece(bitboard_log->bitboards[i], NUMBER_OF_BIBOARD, get_to_square(move_played));
            ...
            // if the white play, we look for the check state of the black player and vice versa
            change_player_that_is_playing(game_settings);
            // at the end we add "+" if it's a check, "#" if it's a checkmate
            int check_state = get_check_state(game_settings->color_of_player_that_is_playing, bitboard_log->bitboards[i], NUMBER_OF_BIBOARD, log, game_settings);
    ...
    // putting the color of the player that have to play back
    game_settings->color_of_player_that_is_playing = color_of_player_have_to_play;
...
void open_file_and_setup_chess_game(bool* has_opened, Log* log, Game_Settings* game_settings, bit_board* bitboards, int nb_bitboards, Bitboards_Log* bitboards_log, Captured_Pieces_and_Score* captured_pieces_and_score){
    ...
    Log* log_copy = create_a_copy_of_log(log);
    ...
    for (int i = 0; i < log_copy->actual_size; i++){
        ...
        if (get_flag(log_copy->moves[i]->move_value) == En_Passant_Flag){
            taken_piece = get_type_of_piece(bitboards, nb_bitboards, (game_settings->color_of_player_that_is_playing == WHITE) ? get_to_square(log_copy->moves[i]->move_value) - 8 : get_to_square(log_copy->moves[i]->move_value) + 8);
        }
        else if (will_capture(game_settings->color_of_player_that_is_playing, bitboards, nb_bitboards, get_to_square(log_copy->moves[i]->move_value))){
            taken_piece = get_type_of_piece(bitboards, nb_bitboards, get_to_square(log_copy->moves[i]->move_value));
        }
        update_captured_piece_and_score(taken_piece, captured_pieces_and_score);

        // making the move
        make_move_AI(game_settings, log, bitboards, nb_bitboards, log_copy->moves[i]->move_value);
