...
int evaluate_position_lvl2_lvl3(bit_board* bitboards, int nb_bitboards){

    int score = 0;
    // getting the score of the pieces
    for (int i=0; i<64; i++){
        int piece = get_type_of_piece(bitboards, nb_bitboards, i);
...
void piece_square_tables_evaluation(bit_board* bitboards, int nb_bitboards, float endgame_white_transition, float endgame_black_transition, int* white_piece_square_tables_score, int* black_piece_square_tables_score){
...
int evaluate_pawns(bit_board* bitboards, int nb_bitboards, int color, int nb_pawns, bit_board friendly_pawn_bitboard, bit_board opponent_pawn_bitboard){
...
int mop_up_score(bit_board* bitboards, int nb_bitboards, int color, int friendly_material, int opponent_material, float opponent_endgame_transition){
...
int king_pawn_shield_score(bit_board* bitboards, int nb_bitboards, int color, int opponent_material, float opponent_endgame_transition, int nb_opponent_queens, int nb_opponent_rooks, int opponent_PST_score){
...
int evaluate(Game_Settings* game_settings, bit_board* bitboards, int nb_bitboards){
    ...
    piece_square_tables_evaluation(bitboards, nb_bitboards, endgame_white_transition, endgame_black_transition, &white_piece_square_tables_score, &black_piece_square_tables_score);
    ...
    score += mop_up_score(bitboards, nb_bitboards, WHITE, white_material, black_material, endgame_black_transition); 
    score -= mop_up_score(bitboards, nb_bitboards, BLACK, black_material, white_material, endgame_white_transition);

    // adding the pawn score, to see if there are passed pawns, isolated pawns, etc.
    score += evaluate_pawns(bitboards, nb_bitboards, WHITE, nb_white_pawns, bitboards[PAWN_WHITE], bitboards[PAWN_BLACK]);
    score -= evaluate_pawns(bitboards, nb_bitboards, BLACK, nb_black_pawns, bitboards[PAWN_BLACK], bitboards[PAWN_WHITE]);

    // adding the king pawn shield score, to see if the king is protected by pawns
    score += king_pawn_shield_score(bitboards, nb_bitboards, WHITE, black_material, endgame_black_transition, nb_black_queens, nb_black_rooks, black_piece_square_tables_score);
    score -= king_pawn_shield_score(bitboards, nb_bitboards, BLACK, white_material, endgame_white_transition, nb_white_queens, nb_white_rooks, white_piece_square_tables_score);
