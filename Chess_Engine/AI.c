...
void make_AI_play(Game_Settings* game_settings, Log* chess_log, Bitboards_Log* bitboards_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score, int level_of_AI, TT* transposition_tables){
    ...
    if (level_of_AI == LVL1){
        ...
        make_AI_random_capture_play(game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score);
    ...
    // level 2 is the alpha_beta pruning method, with a depth we can ajust here
        ...
        int lvl2_depth = 2;
        make_AI_alpha_beta_play(lvl2_depth, game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score);
    ...
    // when the seach is completed, it also does an alpha_beta pruning on the moves that are making a captures until there are no more if possible, or at a certain depth given here, to limit (quiescence search)
        ...
        int lvl3_depth = 2;
        make_AI_alpha_beta_play_quiescence_search(lvl3_depth, game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score);
    ...
    // level 4 is the alpha_beta pruning method, with a cleaner evaluation function, ajusted depending on the phase of the game
        ...
        int lvl4_depth = 2;
        make_AI_alpha_beta_play_quiescence_search_eval_up(lvl4_depth, game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score);
    ...
    // level 5 is the alpha_beta pruning method, with the implementation of the iterative deepening to search deeper and more efficiently, first it will be slower, but then it will be faster with the transposition tables
        ...
        make_AI_alpha_beta_play_iterative_deepening(game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score);
    ...
    // level 6 is the alpha_beta pruning method, with the implementation of the transposition table to fast things up, to be able to search deeper for the same time still with the iterative deepening
       ...
       make_AI_alpha_beta_play_transposition_tables(game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score, transposition_tables);
    ...
    // level 7 is the alpha_beta pruning method, with the implementation of the transposition table to fast things up, to be able to search deeper for the same time still with the iterative deepening, and the implementation extended search where it's critical
        ...
        int time_of_search = 5000; // 5 seconds
        run_iterative_deepening(time_of_search, game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score, &selected_move, transposition_tables);
        ...
        if (get_flag(selected_move) == En_Passant_Flag){
            taken_piece = get_type_of_piece(bitboards, nb_bitboards, (game_settings->color_of_player_that_is_playing == WHITE) ? get_to_square(selected_move) - 8 : get_to_square(selected_move) + 8);
        }
        else if (will_capture(game_settings->color_of_player_that_is_playing, bitboards, nb_bitboards, get_to_square(selected_move))){
            taken_piece = get_type_of_piece(bitboards, nb_bitboards, get_to_square(selected_move));
        }
        update_captured_piece_and_score(taken_piece, captured_pieces_and_score);

        // making the move
        make_move_AI(game_settings, chess_log, bitboards, nb_bitboards, selected_move);

    }

    // adding the biboard to the log to be able to detect draws after
    add_bitboards_to_log(bitboards_log, bitboards);
...
void make_AI_random_capture_play(Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score){
    ...
    move* valid_moves_capture = generate_all_possible_moves_that_capture(&nb_moves_capture, chess_log, game_settings, bitboards, nb_bitboards);
    ...
    for (int i = 0; i < nb_moves_capture; i++){
        ...
        int piece_type_that_take = get_type_of_piece(bitboards, nb_bitboards, valid_move_from);
        ...
        if (is_en_passant_possible(game_settings->color_of_player_that_is_playing, bitboards, nb_bitboards, valid_move_from, valid_move_to, chess_log)){
            piece_type_that_is_taken = get_type_of_piece(bitboards, nb_bitboards, (game_settings->color_of_player_that_is_playing == WHITE) ? valid_move_to - 8 : valid_move_to + 8);
        }
        else {
            piece_type_that_is_taken = get_type_of_piece(bitboards, nb_bitboards, valid_move_to);
        }
...
int Alpha_Beta_LVL2_Search_Valid_Moves(int depth, int lvl2_depth, int alpha, int beta, move* selected_move, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    if (depth == 0){
        ...
        return turncolor * evaluate_position_lvl2_lvl3(bitboards, nb_bitboards);
    }
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        int score = -Alpha_Beta_LVL2_Search_Valid_Moves(depth - 1, lvl2_depth, -beta, -alpha, selected_move, game_settings, chess_log, bitboards, nb_bitboards);
...
void make_AI_alpha_beta_play(int lvl2_depth, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score){
    ...
    int score = Alpha_Beta_LVL2_Search_Valid_Moves(lvl2_depth, lvl2_depth, -MAX_IA_SCORE, MAX_IA_SCORE, &selected_move, game_settings, chess_log, bitboards, nb_bitboards);
    ...
    if (is_move_null(selected_move)){
        ...
        int score = Alpha_Beta_LVL2_Search_Valid_Moves(1, 1, -MAX_IA_SCORE, MAX_IA_SCORE, &selected_move, game_settings, chess_log, bitboards, nb_bitboards);
        printf("lvl2 score : %d for move from %d to %d\n", score, get_from_square(selected_move), get_to_square(selected_move));
        printf("\n");
    }
...
int Alpha_Beta_LVL3_Search_Capture_Moves(int alpha, int beta, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    int turncolor = (game_settings->color_of_player_that_is_playing == WHITE) ? 1 : -1;
    int evaluation = turncolor * evaluate_position_lvl2_lvl3(bitboards, nb_bitboards);
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        make_move_AI(game_settings, chess_log, bitboards, nb_bitboards, valid_moves[i]);
        evaluation = -Alpha_Beta_LVL3_Search_Capture_Moves(-beta, -alpha, game_settings, chess_log, bitboards, nb_bitboards);
        undo_last_move_AI(chess_log, bitboards, nb_bitboards, game_settings);
...
int Alpha_Beta_LVL3_Search_Moves(int depth, int lvl3_depth, int alpha, int beta, move* selected_move, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    if (depth == 0){
        ...
        return Alpha_Beta_LVL3_Search_Capture_Moves(alpha, beta, game_settings, chess_log, bitboards, nb_bitboards);
    }
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        // making the move, getting the score of the move by going one move deeper, and undoing the move
        make_move_AI(game_settings, chess_log, bitboards, nb_bitboards, valid_moves[i]);
        int score = -Alpha_Beta_LVL3_Search_Moves(depth - 1, lvl3_depth, -beta, -alpha, selected_move, game_settings, chess_log, bitboards, nb_bitboards);
        undo_last_move_AI(chess_log, bitboards, nb_bitboards, game_settings);
...
void make_AI_alpha_beta_play_quiescence_search(int lvl3_depth, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score){
    ...
    // making the search for the best move for this AI level
    int score = Alpha_Beta_LVL3_Search_Moves(lvl3_depth, lvl3_depth, -MAX_IA_SCORE, MAX_IA_SCORE, &selected_move, game_settings, chess_log, bitboards, nb_bitboards);
    ...
    if (is_move_null(selected_move)){
        ...
        // we can here see how useful will be the iterative deepening that will be implemented later 
        printf("no move found at first : ");
        score = Alpha_Beta_LVL3_Search_Moves(1, 1, -MAX_IA_SCORE, MAX_IA_SCORE, &selected_move, game_settings, chess_log, bitboards, nb_bitboards);
        printf("lvl3 score : %d for move from %d to %d\n", score, get_from_square(selected_move), get_to_square(selected_move));
        printf("\n");
    }
...
int Alpha_Beta_LVL4_Search_Capture_Moves_eval_up(int alpha, int beta, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        // making the move, getting the score of the position after the move (updating the evaluation), and undoing the move
        make_move_AI(game_settings, chess_log, bitboards, nb_bitboards, valid_moves[i]);
        evaluation = -Alpha_Beta_LVL4_Search_Capture_Moves_eval_up(-beta, -alpha, game_settings, chess_log, bitboards, nb_bitboards);
        undo_last_move_AI(chess_log, bitboards, nb_bitboards, game_settings);
...
int Alpha_Beta_LVL4_Search_Moves_eval_up(int depth, int lvl4_depth, int alpha, int beta, move* selected_move, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    if (depth == 0){
        // making the search for the captures happening afterwards
        return Alpha_Beta_LVL4_Search_Capture_Moves_eval_up(alpha, beta, game_settings, chess_log, bitboards, nb_bitboards);
    }
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        // making the move, getting the score of the move by going one move deeper, and undoing the move
        make_move_AI(game_settings, chess_log, bitboards, nb_bitboards, valid_moves[i]);
        int score = -Alpha_Beta_LVL4_Search_Moves_eval_up(depth - 1, lvl4_depth, -beta, -alpha, selected_move, game_settings, chess_log, bitboards, nb_bitboards);
        undo_last_move_AI(chess_log, bitboards, nb_bitboards, game_settings);
...
void make_AI_alpha_beta_play_quiescence_search_eval_up(int lvl4_depth, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score){
    ...
    // making the search for the best move for this AI level
    int score = Alpha_Beta_LVL4_Search_Moves_eval_up(lvl4_depth, lvl4_depth, -MAX_IA_SCORE, MAX_IA_SCORE, &selected_move, game_settings, chess_log, bitboards, nb_bitboards);
    ...
    if (is_move_null(selected_move)){
        ...
        // we can here see how useful will be the iterative deepening that will be implemented later 
        printf("no move found at first : ");
        score = Alpha_Beta_LVL4_Search_Moves_eval_up(1, 1, -MAX_IA_SCORE, MAX_IA_SCORE, &selected_move, game_settings, chess_log, bitboards, nb_bitboards);
...
void test_iterative_deepening(Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score, move* selected_move){
    ...
    int depth_1_score = Alpha_Beta_LVL5_Search_Moves_iterative_deepening(1, 0, NEGATIVE_INFINITY, POSITIVE_INFINITY, &best_move, &best_score, &is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards);
    ...
    for (int search_depth = 2; search_depth < MAX_DEPTH_ITERATIVE_DEEPENING; search_depth++){
        ...
        // making the search for the best move for this AI level
        int score = Alpha_Beta_LVL5_Search_Moves_iterative_deepening(search_depth, 0, NEGATIVE_INFINITY, POSITIVE_INFINITY, &best_move_this_iteration, &best_score_this_iteration, &is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards);
...
int Alpha_Beta_LVL5_Search_Capture_Moves_iterative_deepening(int alpha, int beta, bool* is_search_canceled, int think_time_limit, int start_time, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        // making the move, getting the score of the position after the move (updating the evaluation), and undoing the move
        make_move_AI(game_settings, chess_log, bitboards, nb_bitboards, valid_moves[i]);
        evaluation = -Alpha_Beta_LVL5_Search_Capture_Moves_iterative_deepening(-beta, -alpha, is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards);
        undo_last_move_AI(chess_log, bitboards, nb_bitboards, game_settings);
...
int Alpha_Beta_LVL5_Search_Moves_iterative_deepening(int ply_remaining, int ply_from_root, int alpha, int beta, move* best_move_this_iteration, int* best_score_this_iteration, bool* is_search_canceled, int think_time_limit, int start_time, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards){
    ...
    if (ply_remaining == 0){
        ...
        return Alpha_Beta_LVL5_Search_Capture_Moves_iterative_deepening(alpha, beta, is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards);
    }
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        int eval = -Alpha_Beta_LVL5_Search_Moves_iterative_deepening(ply_remaining-1, ply_from_root+1, -beta, -alpha, best_move_this_iteration, best_score_this_iteration, is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards);
...
void make_AI_alpha_beta_play_iterative_deepening(Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score){
    ...
    // testing to play with the iterative deepening
    test_iterative_deepening(game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score, &selected_move);
...
void test_iterative_deepening_transposition_tables(Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score, move* selected_move, TT* transposition_tables){
    ...
    // running the depth 1 search without time restriction
    int depth_1_score = Alpha_Beta_LVL6_Search_Moves_transposition_tables(1, 0, NEGATIVE_INFINITY, POSITIVE_INFINITY, &best_move, &best_score, &is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards, transposition_tables);
    printf("score : %d for move from %d to %d for depth 1\n", depth_1_score, get_from_square(best_move), get_to_square(best_move));
    int time_passed = (int)SDL_GetTicks() - start_time;
    ...
    for (int search_depth = 2; search_depth < MAX_DEPTH_ITERATIVE_DEEPENING; search_depth++){
        ...
        // making the search for the best move for this AI level
        int score = Alpha_Beta_LVL6_Search_Moves_transposition_tables(search_depth, 0, NEGATIVE_INFINITY, POSITIVE_INFINITY, &best_move_this_iteration, &best_score_this_iteration, &is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards, transposition_tables);
        printf("score : %d for move from %d to %d for depth %d\n", score, get_from_square(best_move_this_iteration), get_to_square(best_move_this_iteration), search_depth);
...
int Alpha_Beta_LVL6_Search_Moves_transposition_tables(int ply_remaining, int ply_from_root, int alpha, int beta, move* best_move_this_iteration, int* best_score_this_iteration, bool* is_search_canceled, int think_time_limit, int start_time, Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, TT* transposition_tables){
    ...
    for (int i = 0; i < nb_moves; i++){
        ...
        int eval = -Alpha_Beta_LVL6_Search_Moves_transposition_tables(ply_remaining-1, ply_from_root+1, -beta, -alpha, best_move_this_iteration, best_score_this_iteration, is_search_canceled, think_time_limit, start_time, game_settings, chess_log, bitboards, nb_bitboards, transposition_tables);
...
void make_AI_alpha_beta_play_transposition_tables(Game_Settings* game_settings, Log* chess_log, bit_board* bitboards, int nb_bitboards, Captured_Pieces_and_Score* captured_pieces_and_score, TT* transposition_tables){
    ...
    // testing to play with the iterative deepening
    test_iterative_deepening_transposition_tables(game_settings, chess_log, bitboards, nb_bitboards, captured_pieces_and_score, &selected_move, transposition_tables);
