...
void show_chess_board(SDL_Renderer *renderer, bit_board* bitboards, int number_of_bitboards, Log* log, Game_Settings* game_settings, bool is_clicked, SDL_Rect draw_red_boundary_move, bool is_reversed, int selected_piece_index, SDL_Rect piece_movement_rect){
    ...
    if (log->actual_size > 0){
        ...
        from_index_rect.x = get_file(get_from_square(log->moves[log->actual_size-1]->move_value)) * SQUARE_SIZE + WINDOW_LEFT_MARGIN;
        from_index_rect.y = WINDOW_BOARD_SIZE - SQUARE_SIZE - get_rank(get_from_square(log->moves[log->actual_size-1]->move_value)) * SQUARE_SIZE + WINDOW_TOP_MARGIN;
        // if the board is reversed, we need to change the coordinates
        if (is_reversed == true){
            from_index_rect.x = (7 - get_file(get_from_square(log->moves[log->actual_size-1]->move_value))) * SQUARE_SIZE + WINDOW_LEFT_MARGIN;
            from_index_rect.y = WINDOW_BOARD_SIZE - SQUARE_SIZE - (7 - get_rank(get_from_square(log->moves[log->actual_size-1]->move_value))) * SQUARE_SIZE + WINDOW_TOP_MARGIN;
        }
        ...
        to_index_rect.x = get_file(get_to_square(log->moves[log->actual_size-1]->move_value)) * SQUARE_SIZE + WINDOW_LEFT_MARGIN;
        to_index_rect.y = WINDOW_BOARD_SIZE - SQUARE_SIZE - get_rank(get_to_square(log->moves[log->actual_size-1]->move_value)) * SQUARE_SIZE + WINDOW_TOP_MARGIN;
        // if the board is reversed, we need to change the coordinates
        if (is_reversed == true){
            to_index_rect.x = (7 - get_file(get_to_square(log->moves[log->actual_size-1]->move_value))) * SQUARE_SIZE + WINDOW_LEFT_MARGIN;
            to_index_rect.y = WINDOW_BOARD_SIZE - SQUARE_SIZE - (7 - get_rank(get_to_square(log->moves[log->actual_size-1]->move_value))) * SQUARE_SIZE + WINDOW_TOP_MARGIN;
        }
    ...
    if (log->actual_size > 0){
        ...
        if (is_check(WHITE, bitboards, number_of_bitboards, log)){
        ...
        else if (is_check(BLACK, bitboards, number_of_bitboards, log)){
...
void show_trajectory(SDL_Renderer* renderer, bit_board* bitboards, int nb_bitboards, Game_Settings* game_settings, Log* log, bool is_clicked, bool is_click_on_board, int selected_piece_index, bool is_reversed, int color_playing){

    // the click on board needs to be shown
    if (is_clicked && is_click_on_board){

        // getting the type of the piece selected
        int type_piece = get_type_of_piece(bitboards, nb_bitboards, selected_piece_index);

        // if the piece is a white pawn
        if (type_piece == PAWN_WHITE && color_playing == WHITE){
            ...
            // left en passant
            if (is_en_passant_possible(WHITE, bitboards, nb_bitboards, selected_piece_index, selected_piece_index+7, log) && get_file(selected_piece_index) != 0){
                if (is_reversed == false){
                    Draw_Filled_Circle(renderer, get_file(selected_piece_index+7) * SQUARE_SIZE + WINDOW_LEFT_MARGIN + divide_by_two(SQUARE_SIZE), WINDOW_BOARD_SIZE - SQUARE_SIZE - get_rank(selected_piece_index+7) * SQUARE_SIZE + WINDOW_TOP_MARGIN + divide_by_two(SQUARE_SIZE), divide_by_two(divide_by_two(divide_by_two((SQUARE_SIZE)))));
                }
                else {
            ...
            // right en passant
            if (is_en_passant_possible(WHITE, bitboards, nb_bitboards, selected_piece_index, selected_piece_index+9, log) && get_file(selected_piece_index) != 7){
                if (is_reversed == false){
                    Draw_Filled_Circle(renderer, get_file(selected_piece_index+9) * SQUARE_SIZE + WINDOW_LEFT_MARGIN + divide_by_two(SQUARE_SIZE), WINDOW_BOARD_SIZE - SQUARE_SIZE - get_rank(selected_piece_index+9) * SQUARE_SIZE + WINDOW_TOP_MARGIN + divide_by_two(SQUARE_SIZE), divide_by_two(divide_by_two(divide_by_two((SQUARE_SIZE)))));
                }
                else {
        ...
        // if the piece is a black pawn
        else if (type_piece == PAWN_BLACK && color_playing == BLACK){
            ...
            // looking for an en passant move
            // left en passant
            if (is_en_passant_possible(BLACK, bitboards, nb_bitboards, selected_piece_index, selected_piece_index-9, log) && get_file(selected_piece_index) != 0){
                if (is_reversed == false){
                    Draw_Filled_Circle(renderer, get_file(selected_piece_index-9) * SQUARE_SIZE + WINDOW_LEFT_MARGIN + divide_by_two(SQUARE_SIZE), WINDOW_BOARD_SIZE - SQUARE_SIZE - get_rank(selected_piece_index-9) * SQUARE_SIZE + WINDOW_TOP_MARGIN + divide_by_two(SQUARE_SIZE), divide_by_two(divide_by_two(divide_by_two((SQUARE_SIZE)))));
                }
                else {
            ...
            // right en passant
            if (is_en_passant_possible(BLACK, bitboards, nb_bitboards, selected_piece_index, selected_piece_index-7, log) && get_file(selected_piece_index) != 7){
                if (is_reversed == false){
                    Draw_Filled_Circle(renderer, get_file(selected_piece_index-7) * SQUARE_SIZE + WINDOW_LEFT_MARGIN + divide_by_two(SQUARE_SIZE), WINDOW_BOARD_SIZE - SQUARE_SIZE - get_rank(selected_piece_index-7) * SQUARE_SIZE + WINDOW_TOP_MARGIN + divide_by_two(SQUARE_SIZE), divide_by_two(divide_by_two(divide_by_two((SQUARE_SIZE)))));
                }
                else {
        ...
        else if (type_piece == KNIGHT_WHITE && color_playing == WHITE){
        ...
        else if (type_piece == KNIGHT_BLACK && color_playing == BLACK){
        ...
        // the queen is both a bishop and a rook (we need to make this separation so the queen has both trajectories)
            ...
            if ((type_piece == BISHOP_WHITE || type_piece == QUEEN_WHITE) && color_playing == WHITE){
            ...
            if ((type_piece == ROOK_WHITE || type_piece == QUEEN_WHITE) && color_playing == WHITE){
        ...
        // the queen is both a bishop and a rook (we need to make this separation so the queen has both trajectories)
            ...
            if ((type_piece == BISHOP_BLACK || type_piece == QUEEN_BLACK) && color_playing == BLACK){
            ...
            if ((type_piece == ROOK_BLACK || type_piece == QUEEN_BLACK) && color_playing == BLACK){
        ...
        // if the piece is a white king
        else if (type_piece == KING_WHITE && color_playing == WHITE){
            ...
            if (is_castle_possible(WHITE, bitboards, nb_bitboards, selected_piece_index, selected_piece_index+2, log)){
                if (is_reversed == false){
        ...
        // if the piece is a black king
        else if (type_piece == KING_BLACK && color_playing == BLACK){
            ...
            // getting the type of castle that can be done
            // looking for the short castle
            if (is_castle_possible(BLACK, bitboards, nb_bitboards, selected_piece_index, selected_piece_index+2, log)){
                if (is_reversed == false){
...
void show_updated_timer(SDL_Renderer* renderer, int time_player_white, int time_player_black, int player_playing, bool is_reversed){
    ...
    if (player_playing == White_Player){
        // if the board is not reversed
        if (is_reversed == false){
    ...
    if (player_playing == Black_Player){
        // if the board is not reversed
        if (is_reversed == false){
            destRect.y = 0;
            SDL_SetRenderDrawColor(renderer, 230, 230, 230, 255); // white
    ...
    int white_minutes_integer = time_player_white / 60;
    int white_seconds_integer = time_player_white % 60;
    ...
    int black_minutes_integer = time_player_black / 60;
    int black_seconds_integer = time_player_black % 60;
...
void add_move_in_log_render(SDL_Renderer* renderer, int log_index, Log* log, Bitboards_Log* bitboard_log, Game_Settings* game_settings, bit_board* bitboards, int nb_bitboards, int color_playing, int font_size, SDL_Color sdl_color, int middle_x, int middle_y){
    ...
    move move_played = log->moves[log_index]->move_value;
    ...
    if (get_flag(move_played) == Castle_Flag){
        ...
        if (is_promotion_flag(get_flag(move_played))){
            ...
            if (log->moves[log_index]->captured_piece_type != NOTHING){
        ...
        // no promotion, we do the normal thing
            ...
            int piece_moved = get_type_of_piece(bitboard_log->bitboards[log_index], nb_bitboards, get_to_square(move_played));
        ...
        int check_state = get_check_state(1-color_playing, bitboard_log->bitboards[log_index], nb_bitboards, log, game_settings);
...
void show_log(SDL_Renderer* renderer, Log* log, Bitboards_Log* bitboard_log, Game_Settings* game_settings, bit_board* bitboards, int nb_bitboards, bool is_pawn_promotion_happening){
    ...
    if (is_pawn_promotion_happening == false){
        ...
        int max_move_shown = min(log->actual_size, 20);
        for (int i=0; i<max_move_shown; i=i+2){
            ...
            if (log->actual_size > 20){
                int delta = 0;
                if (log->actual_size %2 == 1){
                    delta = 1;
                }
                sprintf(number_move, "%d", (log->actual_size-20+i)/2 + 1 + delta);
            }
            else {
            ...
            if (log->actual_size %2 == 1 && ((i == max_move_shown-1 && max_move_shown != 20) || (i == max_move_shown-2 && max_move_shown == 20))){
                // adding the move of the white player
                if (log->actual_size > 20){
                    add_move_in_log_render(renderer, log->actual_size-20+i+1, log, bitboard_log, game_settings, bitboards, nb_bitboards, WHITE, 30, color, 975, starting_height + i*25);
                }
                else {
                    add_move_in_log_render(renderer, i, log, bitboard_log, game_settings, bitboards, nb_bitboards, WHITE, 30, color, 975, starting_height + i*25);
                }
            }
            else {
                if (log->actual_size > 20){
                    ...
                    // adding the move of the white player
                    add_move_in_log_render(renderer, log->actual_size-20+i+delta, log, bitboard_log, game_settings, bitboards, nb_bitboards, WHITE, 30, color, 975, starting_height + i*25);

                    // adding the move of the black player
                    add_move_in_log_render(renderer, log->actual_size-20+i+1+delta, log, bitboard_log, game_settings, bitboards, nb_bitboards, BLACK, 30, color, 1125, starting_height + i*25);
                }
                else {
                    // adding the move of the white player
                    add_move_in_log_render(renderer, i, log, bitboard_log, game_settings, bitboards, nb_bitboards, WHITE, 30, color, 975, starting_height + i*25);
                    ...
                    // adding the move of the black player
                    add_move_in_log_render(renderer, i+1, log, bitboard_log, game_settings, bitboards, nb_bitboards, BLACK, 30, color, 1125, starting_height + i*25);
                }
            }
...
void show_menu_button_in_game(SDL_Renderer* renderer, Button** Buttons, bool is_pawn_promotion_happening, int color_promoted_pawn, bool is_game_paused, int game_mode){
    ...
    if (game_mode == AI_VS_AI || game_mode == PLAYER_VS_AI){
